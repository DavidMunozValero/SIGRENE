...
export function CoachIndex() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setIsLoading(true);
        const [nadadoresRes] = await Promise.all([
          api.getNadadores(),
        ]);
        ...
        <SectionCard title="Resumen de entrenamiento" description="Datos de la semana">
          <div className="space-y-3">
            <div className="flex justify-between items-center p-3 rounded-lg bg-muted/40">
              <span className="text-sm text-muted-foreground">Sesiones esta semana</span>
              <span className="text-sm font-semibold">0</span>
            </div>
            <div className="flex justify-between items-center p-3 rounded-lg bg-muted/40">
              <span className="text-sm text-muted-foreground">Volumen total</span>
              <span className="text-sm font-semibold">0m</span>
            </div>
          </div>
        </SectionCard>
      </div>
