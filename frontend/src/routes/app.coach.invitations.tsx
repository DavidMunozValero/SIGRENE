...
export const Route = createFileRoute("/app/coach/invitations")({
  head: () => ({ meta: [{ title: "Invitaciones — Entrenador" }] }),
...
function InvitationsPage({ t }: { t: (key: string) => string }) {
  ...
  const handleSendInvitation = async (e: React.FormEvent) => {
    ...
    try {
      await api.crearInvitacion({
        ...
        rol_asignado: "swimmer",
      });
      setSuccess(t("coach.invitations.send_success"));
      setFormData({ email: "" });
      setShowForm(false);
      loadInvitaciones();
    } catch (err: any) {
  ...
  const handleCancelInvitation = async (id: string) => {
    if (!confirm(t("coach.invitations.cancel_confirm"))) return;
    try {
  ...
  return (
    <>
      <PageHeader
        title={t("coach.invitations.title_page")}
        description={t("coach.invitations.desc")}
        action={
          <Button variant="hero" onClick={() => setShowForm(!showForm)}>
            {showForm ? t("coach.invitations.cancel") : "+ " + t("coach.invitations.send")}
          </Button>
        }
      />

      {showForm && (
        <SectionCard title={t("coach.invitations.send")}>
          <form onSubmit={handleSendInvitation} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="email">{t("coach.invitations.email")}</Label>
              <Input
                id="email"
                type="email"
                value={formData.email}
                onChange={(e) => setFormData({ email: e.target.value })}
                placeholder="nadador@club.es"
                required
              />
            </div>
            <p className="text-sm text-muted-foreground">
              {t("coach.invitations.email_help")}
            </p>
            <div className="flex gap-2">
              <Button type="submit" variant="hero" disabled={sending}>
                {sending ? t("coach.invitations.sending") : t("coach.invitations.send")}
              </Button>
              <Button type="button" variant="outline" onClick={() => setShowForm(false)}>
                {t("coach.invitations.cancel")}
              </Button>
            </div>
          </form>
        </SectionCard>
      ...
      <SectionCard title={t("coach.invitations.sent")}>
        {isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>{t("coach.invitations.no_pending")}</p>
          </div>
        ) : (
