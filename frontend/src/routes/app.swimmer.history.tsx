...
function HistoryPage() {
  return (
    <>
      <PageHeader title="Mi historial" description="Tus últimas dos semanas de wellness." />
      <SectionCard title="Tendencia 14 días">
          ...
          {days.reverse().map((d) => (
            <div key={d.d} className="flex-1 flex flex-col items-center gap-1.5">
              <div className="w-full flex flex-col gap-0.5 items-center" style={{ height: "100%" }}>
                <div className="w-full rounded-t bg-gradient-water shadow-aqua" style={{ height: `${d.sleep * 8}%` }} title={`Sueño ${d.sleep}h`} />
              </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-xs uppercase text-muted-foreground tracking-wider">
                <tr>
                  ...
                  <th className="text-left px-5 py-3">Sueño</th>
              ...
              <tbody className="divide-y divide-border/60">
                {days.map((d) => (
                  <tr key={d.d} className="hover:bg-muted/30">
                    <td className="px-5 py-3 font-medium text-foreground">{d.d}</td>
                    <td className="px-5 py-3 text-muted-foreground">{d.sleep} h</td>
                    <td className="px-5 py-3 text-muted-foreground">{d.mood}/10</td>
                    <td className="px-5 py-3 text-muted-foreground">{d.fatigue}/10</td>
                  </tr>
                ))}
              </tbody>
            </table>
